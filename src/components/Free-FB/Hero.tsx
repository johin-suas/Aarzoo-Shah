...
/** ✅ Leads Webhook (FREE) */
const WEBHOOK_URL =
  "https://offbeatn8n.coachswastik.com/webhook/aarzoo-shah";
...
const Hero = () => {
  ...
  const HERO_MOBILE = "/group.jpeg";
  const HERO_DESKTOP = "/group.jpeg";
      ...
      for (const url of CSV_URLS) {
        try {
          ...
          if (rows.length >= 2) {
            ...
            const parsedDate =
              dateIdx >= 0 && data[dateIdx]
                ? cleanText(data[dateIdx])
                : "Date TBA";
  ...
  /** ✅ Form submit → send to leads webhook (FREE) */
  const onSubmit = async (e: React.FormEvent) => {
    
    e.preventDefault();
    
    if (form.phone.length !== 10) {
  setSuccessMsg("⚠️ Please enter a valid 10-digit phone number.");
  setSubmitting(false);
  return;
}
    ...
    // ✅ Pixel lead event (free)
    try {
      if (window.fbq) {
        window.fbq("track", "Lead", {
          content_name: "Free Money Masterclass",
        });
      }
    } catch {
      // ignore
    ...
    // ✅ send to webhook (don’t break UX if it fails)
    try {
      ...
      setSuccessMsg("✅ You’re registered! Check WhatsApp/Email for details.");
    ...
    window.location.assign("/ty-smart-wealth-fb3");
  };
        ...
        >
          ...
          >
            <span className="font-semibold" style={{ color: COLORS.navy }}>
              Join Aarzoo Shah’s 2-Day Masterclass to understand how to make your
              savings work smarter. (100% FREE)
            </span>
          </motion.p>

          <p className="font-base text-red-900 text-xs">
            *Educational content only. This is not financial or investment
            advice.
          </p>
        </motion.div>
        ...
        <motion.div variants={leftCol} className="space-y-4 lg:col-start-1 lg:row-start-2">
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            ...
            >
              ...
              <div>
                <p className="text-[11px] sm:text-xs font-semibold opacity-70">
                  Date
                </p>
                <p className="text-xs sm:text-sm font-bold">21 & 22 February 2026</p>
              </div>
            </motion.div>
          ...
          <div className="rounded-3xl border border-black/10 shadow-[0_14px_30px_rgba(0,0,0,0.12)] bg-[#f5b976] p-6 md:p-8">
            ...
            <form onSubmit={onSubmit} className="space-y-5" id="register">
             ...
             focus:border-black/30 focus:ring-4 focus:ring-black/5"
              ...
              {successMsg ? (
                <div className="text-center text-sm font-semibold">{successMsg}</div>
              ) : null}
            </form>
          </div>
        </motion.div>
